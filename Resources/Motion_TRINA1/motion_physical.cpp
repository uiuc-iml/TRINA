...
const static double reflex_offset [4] = {3,3,3,1.5};
const static double reflex_scaling [4] = {-3,-3,-3,-1.5};
//const static double reflex_offset [4] = { 0,0,0,0};
//const static double reflex_scaling [4] = { 4.5, 4.5, 4.5, 2.5 };
bool New_left_reflex=false;
bool New_right_reflex=false;
...
public:
  ...
  AutoMsg<reflex_msgs::Hand> leftNewReflexGripperState,rightNewReflexGripperState;
  //AutoMsg<reflex_msgs::Hand> leftReflexGripperState;
...
bool MyControllerUpdateData::MyStartup() {
  ...
  leftNewReflexGripperState.subscribe(*nh,(leftPrefix+string("/hand_state")).c_str(),10);
  rightNewReflexGripperState.subscribe(*nh,(rightPrefix+string("/hand_state")).c_str(),10);
  ...
  ros::Publisher baseCmdMux = nh->advertise<std_msgs::String>("hstar_amp1/base/cmd/mode",10);
  ...
  while((timeout--)>0 && !kill && ros::ok()) {
	...
	robotState.leftGripper.positionCommand[i] = (leftReflexGripperState->current_pos[i]-reflex_offset[i])/reflex_scaling[i];
      ...
      //unsubscribe from other gripper messages
      leftGripperState.unsubscribe();
      leftNewReflexGripperState.unsubscribe();
    }
...
//unsubscribe from Baxter messages
      ...
      rightNewReflexGripperState.unsubscribe();
    }
    if(leftReflexGripperState.msgCount > 0 && rightReflexGripperState.msgCount > 0) {
     ...
     if(leftNewReflexGripperState.msgCount > 0) {
      printf("Got New Reflex gripper on left hand\n");
      robotState.leftGripper.enabled = 1;
      robotState.leftGripper.name = "ReFlex";
      robotState.leftGripper.positionCommand.resize(4,0);
      robotState.leftGripper.speedCommand.resize(4,0);
      robotState.leftGripper.forceCommand.resize(4,0);
      for(int i=0;i<4;i++)
	robotState.leftGripper.positionCommand[i] = (leftNewReflexGripperState->motor[i].joint_angle-reflex_offset[i])/reflex_scaling[i];
      left_gripper_pub = nh->advertise<reflex_msgs::PoseCommand>("reflex_hand1/command_position",10,false);
      //unsubscribe from Baxter messages
      leftGripperState.unsubscribe();
      leftReflexGripperState.unsubscribe();
      New_left_reflex=true;
    }
    if(rightNewReflexGripperState.msgCount > 0) {
       printf("Got New Reflex gripper on right hand\n");
      robotState.rightGripper.enabled = 1;
      robotState.rightGripper.name = "ReFlex";
      robotState.rightGripper.positionCommand.resize(4,0);
      robotState.rightGripper.speedCommand.resize(4,0);
      robotState.rightGripper.forceCommand.resize(4,0);
      for(int i=0;i<4;i++)
	robotState.rightGripper.positionCommand[i] = (rightNewReflexGripperState->motor[i].joint_angle-reflex_offset[i])/reflex_scaling[i];
      right_gripper_pub = nh->advertise<reflex_msgs::PoseCommand>("reflex_hand2/command_position",2,false);
//unsubscribe from Baxter messages
      rightGripperState.unsubscribe();
      rightNewReflexGripperState.unsubscribe();
      New_right_reflex=true;
    }
 if(leftNewReflexGripperState.msgCount > 0 && rightNewReflexGripperState.msgCount > 0) {
      //done waiting for grippers
...
bool MyControllerUpdateData::MyProcessSensors() 
{
 ...
 if(leftNewReflexGripperState.changed())  {
    robotState.leftGripper.moving = leftNewReflexGripperState->is_moving;
    robotState.leftGripper.position.resize(4);
    //robotState.leftGripper.speed.resize(4,0);
    robotState.leftGripper.force.resize(4);
    for(int i=0;i<4;i++) {
      robotState.leftGripper.position[i] = (leftNewReflexGripperState->motor[i].joint_angle-reflex_offset[i])/reflex_scaling[i];
      //robotState.leftGripper.speed[i] = leftReflexGripperState->velocity[i];
      robotState.leftGripper.force[i] = leftNewReflexGripperState->motor[i].load;
    }
    leftNewReflexGripperState.markRead();
  }
  if(rightNewReflexGripperState.changed())  {
    robotState.rightGripper.moving = rightNewReflexGripperState->is_moving;
    robotState.rightGripper.position.resize(4);
    //robotState.rightGripper.speed.resize(4,0);
    robotState.rightGripper.force.resize(4);
    for(int i=0;i<4;i++) {
      robotState.rightGripper.position[i] = (rightNewReflexGripperState->motor[i].joint_angle-reflex_offset[i])/reflex_scaling[i];
      //robotState.rightGripper.speed[i] = rightReflexGripperState->velocity[i];
      robotState.rightGripper.force[i] = rightNewReflexGripperState->motor[i].load;
     //robotState.rightGripper.position[i] = (rightReflexGripperState->current_pos[i]-reflex_offset[i])/reflex_scaling[i];
      //robotState.rightGripper.speed[i] = rightReflexGripperState->velocity[i];
     // robotState.rightGripper.force[i] = rightReflexGripperState->load[i];
 }
    rightNewReflexGripperState.markRead();
  }
  return true;
}
