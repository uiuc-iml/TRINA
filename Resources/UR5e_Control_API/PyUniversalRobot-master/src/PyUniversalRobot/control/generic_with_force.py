...
class Controller(object):
    def __init__(self, host, **kwargs):
        ...
        self._tcp = kwargs.pop('tcp', [0]*6)
        self._payload = kwargs.pop('payload', 0)
    ...
    def start(self):
        ...
        conn.send_output_setup(
            ['timestamp', 'target_q', 'actual_q', 'target_qd', 'actual_qd', 'target_qdd', 'target_speed_fraction','actual_TCP_force','tcp_force_scalar'],  # Add TCP forces here
            ['DOUBLE', 'VECTOR6D', 'VECTOR6D', 'VECTOR6D', 'VECTOR6D', 'VECTOR6D', 'DOUBLE', 'VECTOR6D','DOUBLE']
        )
...
class WiggleController(Controller):
    ...
    def update(self, state):
        ...
        self.servo(q=[0,-1.57,-1.57,-1.57,0,0])
        '''
        if t < wait_time:
        ...
        dt = t - self._last_t
        self._last_t = t
        #print('q'+'{:6.4f} {:6.4f}'.format(t, dt), ' '.join(['{:6.2f}'.format(x/pi*180) for x in state.actual_q]))
        print('ft'+'{:6.4f} {:6.4f}'.format(t, dt), ' '.join(['{:6.2f}'.format(x) for x in state.actual_TCP_force])) 
