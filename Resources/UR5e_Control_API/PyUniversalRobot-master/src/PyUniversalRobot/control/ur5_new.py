...
MODES = ["discrete", "continuous", "velocity"]
MODE_DISCRETE = MODES[0]
MODE_CONTINUOUS = MODES[1]
MODE_VELOCITY = MODES[2]
...
class Ur5Controller(object):
    def __init__(self, host, **kwargs):
        ...
        self._velocity_transition = 0
        self._max_velocity_transition = 0

        self._mode = kwargs.pop('mode', MODE_DISCRETE)
        if type(self._mode) == int:
            if self._mode > len(MODES):
                 print "improper mode, setting to discrete"
            else:
                self._mode = MODES[self._mode]
        elif not self._mode in MODES:
            print "improper mode, setting to discrete"
            self._mode = MODE_DISCRETE

        self._log = kwargs.pop('log', False)
        self._logfile = kwargs.pop('logfile', "path_data.txt")

        self._gripper_enable = kwargs.pop('gripper', True)
        self._gripper_delay_t = 0
        self._gripper_width = 1
        self._gripper_ser = None
        #1 is open, 0 is closed

        self._eps = kwargs.pop('eps', 1e-3)
        self._max_eps = 1e-3
        self._speed = kwargs.pop('speed', 3)
        self._max_speed = 6
    ...
    def setMode(self, mode):
        if mode == 0 or mode == "discrete":
            #discrete mode
            self._mode = MODE_DISCRETE
        elif mode == 1 or mode == "continuous":
            self._mode = MODE_CONTINUOUS
        else:
            print "Error, mode ", mode ," not available"
    ...
    def update(self,state):
        ...
        self._qdot_curr = state.actual_qd

        q_curr = self._q_curr
        q_curr.append(self._gripper_width)
        qdot_curr = self._qdot_curr

        if self._run:
            print "waiting on pathlock"
            self._pathLock.acquire()
            print "got pathlock"
            ...
            if self._path and self._last_t > self._gripper_delay_t :
                ...
                if isFormatted(self._path[0]) and isFormatted(q_curr) and inJointLimits(self._path[0] ) :
                    ...
                    if self._mode == MODE_DISCRETE:
                        ...
                        print "in continuous mode"
                        q_commanded = self.continuousUpdate(q_curr, slope, slope_norm, dt)
                    elif self._mode == MODE_VELOCITY:
                        print "in velocity mode"
                        qdot_commanded = self.velocityUpdate(q_curr, qdot_curr, slope, slope_norm, dt)

                    self._q_commanded = [v for v in q_commanded]
                    self._qdot_commanded = [v for v in qdot_commanded]
            ...
            if self._mode == MODE_VELOCITY:
                self.servo(qd=self._qdot_commanded)
                #print('{:6.4f} {:6.4f}'.format(t, dt), ' '.join(['{:6.2f} '.format(x) for x in qdot_curr]))
            else:
                print self._q_commanded
            ...
            print "going to current position"
            self.servo(q=state.actual_q)
    ...
    def continuousUpdate(self, q_curr, slope, slope_norm, dt, smoothing=3):
        q_commanded = []
        ...
        #other milestones
            ...
            if slope_norm <= self._speed*smoothing*dt:
                self._path = self._path[1:]
                new_slope = []
                for i in range(0, len(q_curr)):
    ...
    def velocityUpdate(self, q_curr, qdot_curr, slope, slope_norm, dt, smoothing=6):
        if len(q_curr) == ROBOT_CONFIG_LEN:
            q_curr = q_curr[:-1]
        if(len(self._path)<2 or not (q_curr[-1] == self._path[0][-1])):
            if slope_norm <= self._eps/2:
                #NOTE: BUG - causing robot to stay at zero velocity
                qdot_commanded = [0]*UR5_CONFIG_LEN
            elif slope_norm <= self._speed*dt*(smoothing-1):
                if self._velocity_transition == 0:
                    self._max_velocity_transition = smoothing
                    self._velocity_transition = smoothing
                if self._velocity_transition > 0:
                    qdot_commanded = vectorops.madd(qdot_curr, [0]*UR5_CONFIG_LEN, self._velocity_transition*1.0/self._max_velocity_transition)
                    self._velocity_transition = self._velocity_transition -1
            else:
                qdot_commanded = vectorops.mul(vectorops.div(slope, slope_norm), self._speed)
        else:
            if slope_norm <= self._speed*dt*smoothing:
                if self._velocity_transition > 0:
                    #go to next - we hit this twice quickly
                    self._path = self._path[1:]
                self._max_velocity_transition = smoothing
                self._velocity_transition = smoothing
            if self._velocity_transition > 0 and len(self._path)>=2:
                slope_shortcut = vecotorops.sub(self._path[0], self._path[1])
                vectorops.norm(slope_shortcut)
                q_next = vectorops.madd(self._path[1], vectorops.div(slope_shortcut, shortcut_norm), self._max_velocity_transition-self._velocity_transition)
                q_next_slope = vectorops.sub(q_curr, q_next)
                qdot_commanded = vectorops.mul(vectorops.div(q_next_slope, vectorops.norm(q_next_slope)), self._speed)
                self._velocity_transition = self._velocity_transition -1
            else:
                qdot_commanded = vectorops.mul(vectorops.div(slope, slope_norm), self._speed)

        return qdot_commanded
...
if __name__ == '__main__':
    ...
    ctrl = Ur5Controller(args.robot, mode='discrete', speed=1)
