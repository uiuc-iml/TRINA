...
if __name__=="__main__":
	motion = MotionClient()
	motion.startServer(mode = "Physical", components = ['right_limb','left_limb'])
	motion.startup()
	time.sleep(0.05)
	try:
		...
		B = B.tolist()
		if left_limb.active:
			left_limb.setEETransformImpedance([leftUntuckedRotation.tolist(),leftUntuckedTranslation.tolist()], K, M, B)
		time.sleep(0.05)
	except:
		print("except")
	motion.shutdown()
