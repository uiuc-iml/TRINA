...
class Camera_Robot:
    ...
    """
        ...
        if(self.mode == 'physical'):
            ...
            # reset arms
            ll_position = self.robot.sensedLeftLimbPosition()
            rl_position = self.robot.sensedRightLimbPosition()
            self.simrobot.setLeftLimbPositionLinear(ll_position,0.1)
            self.simrobot.setRightLimbPositionLinear(rl_position,0.1)
    ...
    def get_point_clouds(self,cameras = []):
        ...
        if(self.mode == 'physical'):
            ...
            try:
                Rrotation,Rtranslation = self.robot.sensedRightEETransform()
                Lrotation,Ltranslation = self.robot.sensedLeftEETransform()
    ...
    def update_sim(self):
        ...
        while(True):
            ll_position = self.robot.sensedLeftLimbPosition()
            rl_position = self.robot.sensedRightLimbPosition()
            self.simrobot.setLeftLimbPositionLinear(ll_position,dt)
            self.simrobot.setRightLimbPositionLinear(rl_position,dt)
