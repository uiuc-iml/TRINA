...
class DirectTeleOperation:
	...
	def unifiedControl(self, left_target, right_target):
		...
		min_sing_mag = np.min(s)
		if min_sing_mag < 1e-4:
			# Singular or nearly singular -> need to get LS solution first then
			# adjust in the null space for remaining objectives
			pass
		else:
			# Just directly optimize the remaining objectives subject to
			# achieving the desired twists
			pass
