...
class Calibration:
	...
	def cameraCalibration(self,location,camera_guess,marker_guess):
		...
		if not (location == 'fixed'):
			...
			if location == 'right':
				EE_transforms = take_pictures(camera = sensor_module,robot = self.robot,arm = location,configurations = TRINAConfig.right_calibration_configs, dimx = 480, dimy = 640)
			elif location == 'left':
				EE_transforms = take_pictures(camera = sensor_module,robot = self.robot,arm = location,configurations = TRINAConfig.left_calibration_configs, dimx = 480, dimy = 640)
