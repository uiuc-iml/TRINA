...
class APILayer:
    ...
    - In each of your methods, it is recommended that you implement your API 
      using _redisGet, _redisSet, _redisRpc, _redisRpcNoReply, or
      _moduleCommand. 
      * _redisGet retrieves a key from the Redis database.  (In your API-module,
        you will probably set the same key for users to read.)
      * _redisSet sets a key from the Redis database.  (In your API-module,
        you will probably read from this key to see what users have set.)
      * _redisRpc uses an RPC mechanism to allow other users to call functions
        in your module.  By default, if you implement a method `foo` in your
        module and your API implements `foo(self,*args)` using
        `return self._redisRpc("foo",*args)`, then RPC will call your module's
        `foo` and return the result to the caller.
      * _redisRpcNoReply is the same as _redisRpc but does not request a return
        value.
      * _moduleCommand uses a lightweight, one-way RPC similar to
    ...
    RPC calls typically return a RpcPromise object, which the caller can
    poll using RpcPromise.available() or wait on using
    ...
    def _redisRpc(self,fn,*args,**kwargs):
        """Uses the Redis server to perform an RPC call. Returns a RpcPromise."""
    ...
    def _redisRpcNoReply(self,fn,*args,**kwargs):
        """Uses the Redis server to perform an RPC call with no reply. Returns None."""
        msg = {'fn': fn, 'args': args, 'kwargs':kwargs,'id': None, 'from':self._caller_name}
        self._redis_client.rpush(self._api_name+"_RPC_QUEUE",json.dumps(msg))

    def _moduleCommand(self,fn,*args):
        """Calls a function fn implemented in the module, specifically a function
        defined in the result of moduleCommandObject. fn(*args) is expected to 
...
class APIModule(Module):
    ...
    The API is exposed using the api() call.  This produces a APILayer
    interface that acts as the "frontend" for your module.  If the interface
    accesses shared resources in this module, it must do so in a thread-safe
    manner.  The easiest way to do this is to use the Redis server as an
    intermediary. 
    ...
    def api(self,other_module_name,**jarvis_handles):
        """To implement an API module, subclass must return a APILayer subclass
        that implements the communication from the other module to your module.
    ...
    def moduleCommandObject(self):
        """If the APILayer returned by api() implements functions using
        _moduleCommand, return a pair (api_name,object) object whose methods
        will be called. 
    ...
    def doRpc(self,fn,args,kwargs):
        ...
        fn in self, and executes the arguments as interpreted as python strings.
        """
        if hasattr(self,fn):
    ...
    def getRedisRpc(self):
        """Manual alternative to processRedisRpcs: you can periodically call this
        to get individual RPC requests.  This is useful for long-running RPC calls
        where you do not want to block your module.

        Returns:
            None if there are no pending RPC requests.   Otherwise, returns a
            dict ``request`` with fields "fn", "args", "kwargs", and "id". It
            will also have "from" indicating which module it's coming from,
        ...
        ``request['id']`` back to ``setRedisRpc`` as its first argument, if it
        is not None.
        """
        redis_client = self.jarvis._redis_client
        key = self.apiName() + "_RPC_QUEUE";
    ...
    def setRedisRPC(self,id,reply):
        """Manually replies to a previous request from getRedisRpc."""
        if id is None:
            return
        rpc_node = self.jarvis._state_server[self.apiName()]['RPC_FEEDBACK']
        #rpc_node[msg['id']] = {'MSG':res,'REPLIED':True}
