    ...
    def __bool__(self):
        ...
        if self._tlast is None:
            ...
            if elapsed_time > self.dt:
                ...
                if self.warning_frequency > 0 and self._num_overruns % self.warning_frequency == 0:
                    ave_overrun = self._time_overrun_since_last_warn / self._num_overruns_since_last_warn
                    self.print_warning("{}: exceeded loop time budget {:.4f}s on {}/{} iters, by {:4f}s on average".format(('TimedLooper' if self.name is None else self.name),
                        self.dt,
                ...
                self._tnext += self.dt
                assert self._tnext >= tnow,"Uh... elapsed time is > dt but tnext < tnow: %f, %f, %f"%(elapsed_time,self._tnext,tnow)
        self._iters += 1
...
if __name__ == '__main__':
    ...
    while looper:
        print("looping",looper.iters(),looper.time_elapsed(),"~=",looper.iters()*looper.dt)
